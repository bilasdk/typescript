...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['BILA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BILA_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Bila API.
 */
export class Bila {
  apiKey: string;
   ...
   */
  constructor({
    baseURL = readEnv('BILA_BASE_URL'),
    apiKey = readEnv('BILA_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BilaError(
        "The BILA_API_KEY environment variable is missing or empty; either provide it, or instantiate the Bila client with an apiKey option, like new Bila({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Bila.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('BILA_LOG'), "process.env['BILA_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('BILA_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static BilaError = Errors.BilaError;
